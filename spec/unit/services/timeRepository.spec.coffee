describe "timeRepository", ->
 ...
 Given angular.mock.inject ($injector,  $rootScope, $q, _fmRestRepository_, _cciApiService_, _fmRestModel_, _fmRestList_) -> 
  ...
  @q = $q
  @rootScope = $rootScope
  @subject = $injector.get 'timeRepository', {$q:@q,fmRestModel: @mockModel, fmRestList:@mockList, fmRestRepository:@mockRepo, cciApiService:@mockApi}
 ...
 ###
 # TODO: Modifiy to implement paging and use fmRestList
 ###
 
 
 describe "getAllForStaff() with no page size", ->
  Given ->
   @succeedPromise = true 
 ...
 describe "getAllForStaff() with a page size", ->
  Given ->
   @succeedPromise = true 
   @pagesize = 'anything'
  
  When ->
   @promise = @subject.getAllForStaff(@staff_id, @pagesize)
   @promise.then (data) => @result = data
   @rootScope.$apply()
   
  Then -> expect(@mockApi.get).toHaveBeenCalledWith('layout/Api-Time.json?RFMsF1=staff_id&RFMsV1=' + @staff_id + '&RFMscript=Api-Time.sort&RFMmax=' + @pagesize)
  Then -> expect(@result).toEqual(@expectedList)
 ...
 describe "delete()", ->
  ...
  describe "when getting a successful api response", ->
   Given ->
    @sentpath = '/RESTfm/STEVE/something'
    @expectedCalledPath = 'something'
    @apiResponse = {data:@successfulApiResponse, status:200}
    @succeedPromise = true
     
   When -> 
    @promise = @subject.delete(@sentpath)
    @promise.then (response) => @result = response
    @rootScope.$apply()
    
   Then -> expect(@mockApi.delete).toHaveBeenCalledWith(@expectedCalledPath)
   Then -> expect(@result).toEqual("Your " + @modelName + " was successfully deleted!")
   
  describe "when getting an error api response", ->
   Given ->
    @sentpath = '/RESTfm/STEVE/something'
    @expectedCalledPath = 'something'
    @apiResponse = {data:@errorApiResponse, status:500}
    @succeedPromise = false
